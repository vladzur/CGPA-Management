import { Controller, Post, Body, Patch, Param, Get, Delete, UseGuards, Req, ForbiddenException } from '@nestjs/common';
...
export class UsuariosController {
  ...
  updateRole(
    ...
    return this.usuariosService.updateUserRole(targetUid, role, req.user.uid, req.user.name);
  }

  @Patch(':uid/activar')
  toggleActive(@Param('uid') targetUid: string, @Req() req: any) {
    requireAdmin(req);
    return this.usuariosService.deactivateUser(targetUid, req.user.uid, req.user.name);
  }

  @Delete(':uid')
  delete(@Param('uid') targetUid: string, @Req() req: any) {
    requireAdmin(req);
    return this.usuariosService.deleteUser(targetUid, req.user.uid, req.user.name);
  }
}
